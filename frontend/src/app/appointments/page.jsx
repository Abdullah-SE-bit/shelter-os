...
export default function AppointmentsPage() {
  ...
  const handleConfirm = () => { if (!selectedAppointment) return; updateSelected({ status: 'CONFIRMED' }); setConfirmDialogOpen(false); setSelectedAppointment(null); };
  const handleCancel = () => { if (!selectedAppointment) return; updateSelected({ status: 'CANCELLED', cancellation_reason: cancellationReason }); setCancelDialogOpen(false); setSelectedAppointment(null); setCancellationReason(''); };
  const handleComplete = () => { if (!selectedAppointment) return; updateSelected({ status: 'COMPLETED', outcome_summary: outcomeSummary }); setCompleteDialogOpen(false); setSelectedAppointment(null); setOutcomeSummary(''); };
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              {isVet && appointment.owner && (
                <div>
                  ...
                  <div className="text-sm font-semibold text-foreground">{appointment.owner_name || '—'}</div>
                </div>
              )}
        ...
        <form id="book-form" onSubmit={handleBookAppointment} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <NativeSelect required value={bookForm.vet} onChange={(e) => setBookForm({ ...bookForm, vet: e.target.value })} className="mt-1.5">
              ...
              {vets.map((vet) => <option key={vet.id} value={vet.id}>Dr. {vet.profile.first_name} {vet.profile.last_name}</option>)}
            </NativeSelect>
          </div>
          <div>
