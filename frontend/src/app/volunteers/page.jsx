...
export default function VolunteerListPage() {
  ...
  const volunteers = filtered.slice((page - 1) * pageSize, page * pageSize);
      ...
      {volunteers.length > 0 && (
        <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {volunteers.map((vol) => {
            ...
            return (
              <Link key={vol.id} href={`/volunteers/${vol.id}`} className="rounded-xl border border-border bg-card p-5 text-center shadow-sm transition-shadow hover:shadow-md">
