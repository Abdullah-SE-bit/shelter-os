...
export default function WeightTrackerPage() {
  ...
  const [logs, setLogs] = useState(() => mockWeightLogs.filter((l) => l.cat === catId));
  ...
  const handleSave = (e) => {
    ...
    setSaving(true);
    setTimeout(() => {
      setLogs((l) => [...l, { id: `w-${Date.now()}`, cat: catId, weight_kg: Number(form.weight_kg), recorded_at: new Date(form.measured_at).toISOString(), notes: form.notes }]);
      setAddOpen(false);
      setSaving(false);
    }, 350);
  };
