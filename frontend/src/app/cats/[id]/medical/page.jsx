...
import { mockMedicalRecords, byCat } from '@/lib/mock-data/medical';
...
export default function MedicalHistoryPage() {
  ...
  const [records, setRecords] = useState(() => byCat(mockMedicalRecords, catId));
    ...
    <div className="mx-auto max-w-[720px] px-4 py-6 sm:px-6">
      ...
      {isUntreatable && (
        <div className="mb-6 flex items-start gap-2.5 rounded-xl border border-warning/25 bg-warning/10 px-4 py-3.5 text-sm font-medium text-warning">
          ...
          <span>This animal is marked as <strong>{(cat?.current_status || '').toLowerCase()}</strong>, so new medical records can't be added.</span>
        </div>
      )}
      ...
      <AddMedicalRecordModal open={addOpen} catId={catId} onClose={() => setAddOpen(false)} onSaved={(rec) => { setAddOpen(false); setRecords((r) => [rec, ...r]); }} />
    </div>
  );
}
...
function AddMedicalRecordModal({ open, catId, onClose, onSaved }) {
  ...
  const handleSave = (e) => {
    ...
    setSaving(true);
    setTimeout(() => {
      onSaved({ id: `mr-${Date.now()}`, cat: catId, ...form, occurred_at: new Date(form.occurred_at).toISOString() });
      setSaving(false);
    }, 350);
  };
