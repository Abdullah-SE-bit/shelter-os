...
export default function EditCatPage() {
  ...
  const [form, setForm] = useState(() => ({
    name: cat?.name || '', gender: cat?.gender || 'UNKNOWN', color: cat?.color || '',
    age_years: cat?.age_years ?? '', age_months: cat?.age_months ?? '',
    current_status: cat?.current_status || 'IN_SHELTER', description: cat?.description || '',
    adoption_fee: cat?.adoption_fee ?? '', adoption_requirements: cat?.adoption_requirements || '',
    is_neutered: !!cat?.is_neutered, is_vaccinated_core: !!cat?.is_vaccinated_core, is_microchipped: !!cat?.is_microchipped,
    is_dewormed: !!cat?.is_dewormed, is_fiv_positive: !!cat?.is_fiv_positive, is_felv_positive: !!cat?.is_felv_positive,
    shelter: cat?.shelter_id || '',
  }));
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setSaving(true);
    setTimeout(() => { setSaving(false); router.push(`/cats/${id}`); }, 350);
  };

  if (!cat) return <div className="p-8"><div className="rounded-lg border border-destructive/20 bg-destructive/10 p-3 text-sm font-medium text-destructive">Animal not found.</div></div>;
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-6 sm:px-6">
      <PageHeader title={`Edit: ${form.name || 'Animal'}`} backTo={`/cats/${id}`} backLabel="Animal profile" />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex gap-3">
          <Button type="button" variant="secondary" onClick={() => router.push(`/cats/${id}`)} className="flex-1">Cancel</Button>
