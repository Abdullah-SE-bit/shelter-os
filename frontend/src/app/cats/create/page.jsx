...
export default function CreateCatPage() {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setTimeout(() => { setLoading(false); router.push('/cats'); }, 400);
  };
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-6 sm:px-6">
      <PageHeader title="Add new animal" description={user?.role === 'VOLUNTEER' ? 'Add an animal to your shelter' : 'Create an animal profile in the system'} backTo="/cats" backLabel="Animals" />
            ...
            <div className="flex flex-col gap-3.5">
              ...
              {(user?.role === 'SUPER_ADMIN' || user?.role === 'VET') && (
                <div>
                  ...
                  <NativeSelect id="cat-shelter" value={form.shelter} onChange={(e) => set('shelter', e.target.value)} className="mt-1.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label htmlFor="cat-years">Age (years)</Label><Input id="cat-years" type="number" min="0" max="30" value={form.age_years} onChange={(e) => set('age_years', e.target.value)} placeholder="2" className="mt-1.5" /></div>
                <div><Label htmlFor="cat-months">Age (months)</Label><Input id="cat-months" type="number" min="0" max="11" value={form.age_months} onChange={(e) => set('age_months', e.target.value)} placeholder="6" className="mt-1.5" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label htmlFor="cat-breed">Breed</Label>
                  <NativeSelect id="cat-breed" value={form.breed} onChange={(e) => set('breed', e.target.value)} className="mt-1.5">
                    {mockBreeds.map((b) => <option key={b.id} value={b.id}>{b.display_label}</option>)}
                  </NativeSelect>
                </div>
                <div><Label htmlFor="cat-color">Color / markings</Label><Input id="cat-color" value={form.color} onChange={(e) => set('color', e.target.value)} placeholder="Orange tabby" className="mt-1.5" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label htmlFor="cat-weight">Weight (kg)</Label><Input id="cat-weight" type="number" step="0.01" min="0.1" value={form.weight_kg} onChange={(e) => set('weight_kg', e.target.value)} placeholder="4.2" className="mt-1.5" /></div>
                <div>
              ...
              <div><Label htmlFor="cat-intake">Intake date</Label><Input id="cat-intake" type="date" value={form.intake_date} onChange={(e) => set('intake_date', e.target.value)} className="mt-1.5" /></div>
              <div><Label htmlFor="cat-desc">Description / personality</Label><Textarea id="cat-desc" value={form.description} onChange={(e) => set('description', e.target.value)} rows={3} placeholder="Describe this animal's personality, habits, and any special needs…" className="mt-1.5" /></div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="flex flex-col gap-3.5">
                <div><Label htmlFor="cat-fee">Adoption fee (PKR)</Label><Input id="cat-fee" type="number" min="0" value={form.adoption_fee} onChange={(e) => set('adoption_fee', e.target.value)} placeholder="0 for free" className="mt-1.5" /></div>
                <div><Label htmlFor="cat-req">Adoption requirements</Label><Textarea id="cat-req" value={form.adoption_requirements} onChange={(e) => set('adoption_requirements', e.target.value)} rows={3} placeholder="Minimum requirements for adopting this animal…" className="mt-1.5" /></div>
              </div>
            </div>
          </>
        )}
