...
import { PawPrint, Heart, Home as HomeIcon, Briefcase, ClipboardList, LifeBuoy } from 'lucide-react';
...
export default function ApplicationFormPage() {
  ...
  const [form, setForm] = useState({
    pet: petId, motivation: '', living_situation: 'HOUSE', has_outdoor_access: false, has_other_pets: false,
    other_pets_details: '', has_children: false, children_ages: '', experience_with_pets: '', employment_status: 'EMPLOYED',
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-6 sm:px-6">
      <PageHeader title="Adoption application" description="Tell us about yourself and your home" backTo={`/adoption/${petId}`} backLabel="Animal profile" />
          ...
          <>
            <Card>
              ...
              <CardContent className="flex flex-col gap-3.5">
                ...
                {form.has_other_pets && (
                  <div><Label>Other pets details</Label><Input value={form.other_pets_details} onChange={(e) => set('other_pets_details', e.target.value)} placeholder="e.g. 1 dog (lab, very friendly), 2 pets" className="mt-1.5" /></div>
                )}
                {form.has_children && (
            ...
            <Card>
              <CardHeader><CardTitle className="flex items-center gap-2"><PawPrint className="size-4" />Animal experience</CardTitle></CardHeader>
              <CardContent>
                <Label>Your experience with animals *</Label>
                <Textarea required value={form.experience_with_pets} onChange={(e) => set('experience_with_pets', e.target.value)} placeholder="Describe your past experience with pets, any training, etc." rows={3} className="mt-1.5" />
              </CardContent>
            </Card>
          </>
        )}
