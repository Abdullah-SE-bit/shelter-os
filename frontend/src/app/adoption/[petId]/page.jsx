...
import { formatCurrency, formatPetAge, petGenderLabel } from '@/utils/formatters';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

const PET_PLACEHOLDER = 'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?w=600&q=80';

export default function PetAdoptionDetailPage() {
  ...
  const listing = getAdoptionListing(petId);
  const pet = getPetById(petId) || listing;
  ...
  const healthBadges = [
    ...
    { ok: pet.is_vaccinated_core, label: 'Core vaccinations' },
    { ok: pet.is_microchipped, label: 'Microchipped' },
    { ok: pet.is_dewormed, label: 'Dewormed' },
    { ok: !pet.is_fiv_positive, label: 'FIV negative' },
    { ok: !pet.is_felv_positive, label: 'FeLV negative' },
  ];

  const quickFacts = [
    { Icon: Cake, label: 'Age', value: formatPetAge(pet.age_years, pet.age_months) },
    { Icon: VenusAndMars, label: 'Gender', value: petGenderLabel(pet.gender) },
    { Icon: Palette, label: 'Color', value: pet.color },
    { Icon: Bone, label: 'Breed', value: pet.breed_label || 'Mixed' },
    { Icon: Home, label: 'Shelter', value: pet.shelter_name },
  ].filter((f) => f.value);

  return (
    <div>
      <div className="relative h-[380px] overflow-hidden bg-surface-muted">
        <img src={mainPhoto} alt={pet.name} className="size-full object-cover" onError={(e) => { e.currentTarget.src = PET_PLACEHOLDER; }} />
        ...
        <div className="absolute right-8 bottom-8 left-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="font-display mb-1 text-[2.5rem] font-bold text-white">{pet.name}</h1>
            <p className="text-[17px] text-white/85">{pet.breed_label || 'Mixed breed'} · {petGenderLabel(pet.gender)} · {formatPetAge(pet.age_years, pet.age_months)}</p>
          </div>
          <div className="text-right">
            <div className="text-[28px] font-black text-white">{pet.adoption_fee > 0 ? formatCurrency(pet.adoption_fee) : 'Free'}</div>
      ...
      <div className="mx-auto max-w-[900px] px-4 py-8 sm:px-6">
        <div className="mb-8 flex flex-wrap gap-3">
          {canApply && <Button size="lg" className="h-12 flex-1 text-base" asChild><Link href={user ? `/adoption/apply/${petId}` : '/login'}><Heart className="size-4" />Apply to adopt {pet.name}</Link></Button>}
          ...
          <div className="flex flex-col gap-5">
            {pet.description && (
              <div className="rounded-xl border border-border bg-card p-5">
                <h2 className="mb-3 text-[17px] font-bold text-foreground">About {pet.name}</h2>
          ...
          <div className="flex flex-col gap-4">
            ...
            {pet.adoption_requirements && (
              <div className="rounded-xl border border-warning/25 bg-warning/10 p-4">
                ...
                <p className="text-sm leading-relaxed text-foreground">{pet.adoption_requirements}</p>
              </div>
            )}
            ...
            <div className="rounded-xl bg-gradient-to-br from-[var(--brand-rust)] to-[var(--brand-ink)] p-5 text-center text-white">
              ...
              {canApply && (
                <Button variant="secondary" asChild><Link href={user ? `/adoption/apply/${petId}` : '/login'}>Apply now<ArrowRight className="size-3.5" /></Link></Button>
              )}
            </div>
          </div>
        </div>
