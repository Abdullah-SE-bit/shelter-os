...
import { mockCampaigns } from '@/lib/mock-data/finance';
...
export default function CampaignsPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', target_amount: '', start_date: new Date().toISOString().split('T')[0], end_date: '', is_active: true, shelter: '', scope: 'SHELTER' });
  const [saving, setSaving] = useState(false);

  const total = campaigns.length;
  const totalPages = Math.ceil(total / pageSize);
  const pageItems = campaigns.slice((page - 1) * pageSize, page * pageSize);
  ...
  const handleSave = (e) => {
    ...
    setTimeout(() => {
      ...
      setCampaigns((c) => [{
        id: `camp-${Date.now()}`, title: form.title, description: form.description, target_amount: Number(form.target_amount),
        collected_amount: 0, status: form.is_active ? 'ACTIVE' : 'INCOMPLETE', start_date: form.start_date, end_date: form.end_date,
        is_app_level: form.scope === 'APP', shelter_name: form.scope === 'SHELTER' ? shelter?.name : null,
      }, ...c]);
      setAddOpen(false);
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6">
      <PageHeader
        ...
        description={`${total} campaigns · Raise funds for animals in need`}
      ...
      {pageItems.length > 0 && (
        <div className="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {pageItems.map((camp) => {
            ...
            return (
              <div key={camp.id} className={cn('flex flex-col overflow-hidden rounded-xl border bg-card shadow-sm', isIncomplete ? 'border-destructive/30' : isOver ? 'border-success/30' : 'border-border')}>
                <div className={cn('relative overflow-hidden px-5 py-4', isIncomplete ? 'bg-destructive' : isOver ? 'bg-success' : 'bg-primary')}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-[13px] leading-snug text-white/75">{camp.description?.slice(0, 60)}{camp.description?.length > 60 ? '…' : ''}</p>
                    </div>
      ...
      <Modal
        ...
        footer={<><Button variant="secondary" onClick={() => setAddOpen(false)}>Cancel</Button><Button form="camp-form" type="submit" disabled={saving}>{saving ? 'Creating…' : 'Create campaign'}</Button></>}
      >
        <form id="camp-form" onSubmit={handleSave} className="flex flex-col gap-3.5">
          ...
          <div>
            ...
            <Input id="camp-title" required value={form.title} onChange={(e) => set('title', e.target.value)} placeholder="e.g. Winter Animal Care Fund 2027" className="mt-1.5" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label htmlFor="camp-goal">Goal amount (PKR) *</Label><Input id="camp-goal" required type="number" min="1" value={form.target_amount} onChange={(e) => set('target_amount', e.target.value)} placeholder="100000" className="mt-1.5" /></div>
            <div><Label htmlFor="camp-start">Start date *</Label><Input id="camp-start" required type="date" value={form.start_date} onChange={(e) => set('start_date', e.target.value)} className="mt-1.5" /></div>
            <div><Label htmlFor="camp-end">End date *</Label><Input id="camp-end" required type="date" value={form.end_date} onChange={(e) => set('end_date', e.target.value)} className="mt-1.5" /></div>
