...
import { mockCities } from '@/lib/mock-data/shelters';
...
// react-leaflet touches `window` at import time — load client-side only.
...
export default function CreateShelterPage() {
  ...
  const users = [...mockUsers.filter((u) => u.role === 'SHELTER_ADMIN'), ...extraOwners];

  const selectedCity = mockCities.find((c) => c.display_label === form.city) || null;
  const cityCenter = selectedCity?.metadata ? { lat: selectedCity.metadata.lat, lng: selectedCity.metadata.lng, radius_km: selectedCity.metadata.radius_km || 45 } : null;
  ...
  const handleCreateOwner = (e) => {
    ...
    setOwnerError('');
    setTimeout(() => {
      const newUser = { id: `new-owner-${Date.now()}`, email: ownerForm.email, role: 'SHELTER_ADMIN', profile: { first_name: ownerForm.first_name, last_name: ownerForm.last_name } };
      setExtraOwners((o) => [...o, newUser]);
      setForm((prev) => ({ ...prev, admin: newUser.id }));
      setOwnerOpen(false);
  ...
  const handleSubmit = (e) => {
    ...
    setSaving(true);
    setTimeout(() => { setSaving(false); router.push('/shelters'); }, 400);
  };
    ...
    <div className="mx-auto max-w-[860px] px-4 py-6 sm:px-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <NativeSelect id="shelter-city" required value={form.city} onChange={(e) => handleChange('city', e.target.value)} className="mt-1.5" aria-invalid={!!errors.city}>
                  ...
                  {mockCities.map((c) => <option key={c.id} value={c.display_label}>{c.display_label}</option>)}
                </NativeSelect>
              ...
              <div>
                <Label htmlFor="shelter-capacity">Capacity (total animals) *</Label>
              ...
              <div>
                ...
                <NativeSelect id="shelter-admin" value={form.admin} onChange={(e) => handleChange('admin', e.target.value)} className="mt-1.5">
                  ...
                  {users.map((u) => (
                    <option key={u.id} value={u.id}>{u.email}{u.profile?.first_name && ` — ${u.profile.first_name} ${u.profile.last_name}`}</option>
                  ))}
                </NativeSelect>
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="secondary" onClick={() => router.push('/shelters')}>Cancel</Button>
