...
import { formatPetAge, statusLabel } from '@/utils/formatters';
...
export default function PetListPage() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6">
      ...
      {pets.length === 0 && (
        <EmptyState icon={PawPrint} title="No animals found" message="Try adjusting your search filters."
          action={canCreate && <Button asChild><Link href={createPath}><Plus className="size-4" />Add first animal</Link></Button>} />
      )}

      {view === 'grid' && pets.length > 0 && (
        <div className="mb-6 grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
          {pets.map((pet) => (
            <Link key={pet.id} href={`/pets/${pet.id}`} className="group overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-shadow hover:shadow-md">
              <div className="relative h-40 overflow-hidden">
                ...
                />
                <div className="absolute top-2 right-2"><StatusBadge status={pet.current_status} /></div>
              </div>
              <div className="p-3.5">
                <h3 className="text-sm font-bold text-foreground">{pet.name || 'Unnamed'}</h3>
                <p className="mt-0.5 text-xs font-medium text-muted-foreground">{pet.breed_label || 'Mixed'} · {formatPetAge(pet.age_years, pet.age_months)}</p>
                {pet.shelter_name ? (
                  <p className="mt-1.5 text-xs text-muted-foreground">{pet.shelter_name}</p>
                ) : pet.owner_name && <p className="mt-1.5 text-xs text-muted-foreground">{pet.owner_name}</p>}
              </div>
            </Link>
          ))}
        </div>
                ...
                <TableRow key={pet.id}>
                  <TableCell>
                    <div className="flex items-center gap-2.5">
                      <img src={pet.primary_photo_url || PET_PLACEHOLDER} alt="" className="size-9 shrink-0 rounded-md object-cover" onError={(e) => { e.currentTarget.src = PET_PLACEHOLDER; }} />
                      <span className="font-semibold text-foreground">{pet.name || 'Unnamed'}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-muted-foreground">{pet.breed_label || 'Mixed'}</TableCell>
                  <TableCell className="text-muted-foreground">{formatPetAge(pet.age_years, pet.age_months)}</TableCell>
                  <TableCell><StatusBadge status={pet.current_status} /></TableCell>
                  <TableCell className="text-sm text-muted-foreground">{pet.shelter_name || pet.owner_name || '—'}</TableCell>
                  <TableCell><Button size="sm" variant="secondary" asChild><Link href={`/pets/${pet.id}`}>View</Link></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
