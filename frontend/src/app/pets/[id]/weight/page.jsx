...
import { mockWeightLogs } from '@/lib/mock-data/wellness';
...
export default function WeightTrackerPage() {
  ...
  const [logs, setLogs] = useState(() => mockWeightLogs.filter((l) => l.pet === petId));
  ...
  const handleSave = (e) => {
    ...
    setSaving(true);
    setTimeout(() => {
      setLogs((l) => [...l, { id: `w-${Date.now()}`, pet: petId, weight_kg: Number(form.weight_kg), recorded_at: new Date(form.measured_at).toISOString(), notes: form.notes }]);
      setAddOpen(false);
      setSaving(false);
    }, 350);
  };
  ...
  return (
    <div className="mx-auto max-w-[720px] px-4 py-6 sm:px-6">
      ...
      {logs.length === 0 && (
        <EmptyState icon={Scale} title="No weight logs" message="Start tracking this animal's weight to monitor their health."
      ...
      {logs.length > 0 && (
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          <Table>
            ...
            <TableBody>
              {[...logs].sort((a, b) => new Date(b.recorded_at) - new Date(a.recorded_at)).map((log, i, arr) => {
                const p = arr[i + 1];
                const change = p ? (log.weight_kg - p.weight_kg).toFixed(2) : null;
                return (
                  <TableRow key={log.id}>
                    <TableCell className="font-semibold">{formatDate(log.recorded_at)}</TableCell>
