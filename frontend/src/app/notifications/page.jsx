...
export default function NotificationsPage() {
  ...
  const handleMarkAsRead = (id) => setNotifications((ns) => ns.map((n) => (n.id === id ? { ...n, is_read: true, read_at: new Date().toISOString() } : n)));
  const handleMarkAllRead = () => setNotifications((ns) => ns.map((n) => (n.is_read ? n : { ...n, is_read: true, read_at: new Date().toISOString() })));
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1.5 flex items-start justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {!notification.is_read && <Button size="sm" variant="outline" onClick={() => handleMarkAsRead(notification.id)}>Mark read</Button>}
            </div>
          </div>
