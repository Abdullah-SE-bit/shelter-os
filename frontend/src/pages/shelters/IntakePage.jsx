...
export default function IntakePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await sheltersApi.createIntake(shelterId || 'me', form);
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-6 sm:px-6">
      <PageHeader title="Cat intake" description="Record a new cat arriving at the shelter" backTo="/shelter/dashboard" backLabel="Dashboard" />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <Card>
          <CardHeader><CardTitle>Cat information</CardTitle></CardHeader>
          <CardContent className="flex flex-col gap-3.5">
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="intake-name">Cat name (or "Unknown")</Label><Input id="intake-name" value={form.cat_name} onChange={(e) => set('cat_name', e.target.value)} placeholder="Whiskers / Unknown" className="mt-1.5" /></div>
            ...
            <div><Label htmlFor="intake-condition">Condition on arrival *</Label><Textarea id="intake-condition" required value={form.condition_on_arrival} onChange={(e) => set('condition_on_arrival', e.target.value)} rows={3} placeholder="Describe the cat's physical condition when it arrived…" className="mt-1.5" /></div>
        ...
        {form.reason === 'STRAY' && (
          <Card>
            ...
            <CardContent>
              <Label htmlFor="intake-loc">Where was the cat found?</Label>
