...
export default function HealthAlertsPage() {
  ...
  const { data, loading, refetch } = useApi(
    () => isVet
      ? wellnessApi.vetHealthAlerts({ type: filterType !== 'ALL' ? filterType : undefined, severity: filterSeverity !== 'ALL' ? filterSeverity : undefined })
  ...
  return (
    <div className="mx-auto max-w-[900px] px-4 py-6 sm:px-6">
      <PageHeader title="Health alerts" description={isVet ? 'System-wide health monitoring alerts' : 'Health alerts for your cats'} />
      ...
      {!loading && alerts.length === 0 && (
        <EmptyState icon={CheckCircle2} title="No health alerts" message={isVet ? 'No health alerts found in the system. All cats appear to be healthy!' : 'No health alerts for your cats. They appear to be healthy!'} />
      )}
      ...
      {!loading && alerts.length > 0 && (
        <>
          <div className="mb-5 flex items-center gap-2 rounded-xl border border-border bg-card px-5 py-3.5 text-sm text-muted-foreground">
            Showing <strong className="text-foreground">{alerts.length}</strong> health alert{alerts.length !== 1 ? 's' : ''}
          </div>
                  ...
                  <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
                    ...
                    {alert.cat && (
                      <Button size="sm" variant="secondary" asChild>
                        <Link to={`/cats/${alert.cat}`}><Cat className="size-3.5" />{alert.cat_name || 'View cat'}<ChevronRight className="size-3.5" /></Link>
                      </Button>
                    )}
                  </div>
