...
import { Frown, Heart, Scissors, Syringe, Radio, Pill, Dna, Cake, VenusAndMars, Palette, Bone, Home, ArrowRight } from 'lucide-react';
...
export default function CatAdoptionDetailPage() {
  ...
  if (!cat) return (
    <div className="flex flex-col items-center gap-3 p-16 text-center">
      ...
      <Button variant="secondary" asChild><Link to="/adoption">Back to adoption</Link></Button>
    </div>
  );
    ...
    <div>
      ...
      <div className="mx-auto max-w-[900px] px-4 py-8 sm:px-6">
        <div className="mb-8 flex flex-wrap gap-3">
          {canApply && <Button size="lg" className="h-12 flex-1 text-base" asChild><Link to={user ? `/adoption/apply/${catId}` : '/login'}><Heart className="size-4" />Apply to adopt {cat.name}</Link></Button>}
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[1.4fr_1fr]">
          <div className="flex flex-col gap-5">
            ...
            {photos.length > 1 && (
              <div className="rounded-xl border border-border bg-card p-5">
                <h2 className="mb-3.5 text-xs font-bold tracking-wide text-muted-foreground uppercase">More photos</h2>
                <div className="grid grid-cols-3 gap-2">
                  {photos.map((p, i) => (
                    <div key={i} className="h-[100px] overflow-hidden rounded-lg">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-xl bg-gradient-to-br from-[var(--brand-rust)] to-[var(--brand-ink)] p-5 text-center text-white">
              ...
              {canApply && (
                <Button variant="secondary" asChild><Link to={user ? `/adoption/apply/${catId}` : '/login'}>Apply now<ArrowRight className="size-3.5" /></Link></Button>
              )}
            </div>
          </div>
        </div>
