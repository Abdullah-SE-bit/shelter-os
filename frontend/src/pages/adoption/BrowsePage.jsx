...
function CatCard({ cat }) {
  return (
    <Link to={`/adoption/${cat.cat}`} className="group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-shadow hover:shadow-md">
...
export default function BrowsePage() {
  ...
  const { data, loading } = useApi(() => adoptionApi.browse({ page, page_size: pageSize, gender, is_neutered: neutered || undefined }), null, [page, gender, neutered]);
    ...
    <div>
      <section className="relative overflow-hidden bg-gradient-to-br from-[var(--brand-ink)] via-[var(--brand-rust)] to-[var(--brand-amber)] px-6 pt-16 pb-20">
        <div className="relative mx-auto max-w-[1200px]">
          ...
          <div className="flex flex-wrap gap-8">
            {[{ Icon: Cat, label: `${total || '—'} cats available` }, { Icon: Home, label: 'Verified shelters' }, { Icon: Heart, label: 'Happy adoptions' }].map(({ Icon, label }) => (
      ...
      <div className="mx-auto max-w-[1200px] px-4 py-10 sm:px-6">
        <div className="mb-8 flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-4 shadow-sm">
          ...
          <div className="ml-auto text-sm font-semibold text-muted-foreground">{loading ? '…' : `${total} cats found`}</div>
        </div>

        {loading && <LoadingSpinner size="lg" text="Finding cats for you…" />}

        {!loading && listings.length === 0 && (
          <EmptyState icon={Home} title="No cats available right now" message="Check back soon — shelters are regularly adding new cats ready for adoption."
            action={!user && <Button asChild><Link to="/register">Create account to get notified</Link></Button>} />
        )}

        {!loading && listings.length > 0 && (
          <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {listings.map((cat) => <CatCard key={cat.id} cat={cat} />)}
          </div>
        )}
        ...
        {!user && listings.length > 0 && (
          <div className="relative mt-12 flex flex-wrap items-center justify-between gap-6 overflow-hidden rounded-2xl bg-gradient-to-br from-[var(--brand-rust)] to-[var(--brand-ink)] p-10">
            ...
            <Button variant="secondary" size="lg" asChild><Link to="/register">Create free account</Link></Button>
          </div>
        )}
      </div>
    </div>
