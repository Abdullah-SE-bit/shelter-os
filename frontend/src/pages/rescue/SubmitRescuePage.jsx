...
const URGENCY_OPTIONS = [
  { value: 'LOW', label: 'Low', desc: 'Cat is safe but needs help' },
  { value: 'MEDIUM', label: 'Medium', desc: 'Cat needs attention soon' },
  { value: 'HIGH', label: 'High', desc: 'Cat is in danger or injured' },
...
export default function SubmitRescuePage() {
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-6 sm:px-6">
      <PageHeader title="Submit rescue report" description="Help us locate and rescue a cat in need" backTo="/rescue" backLabel="Rescue" />
        ...
        <Card>
          <CardContent className="flex flex-col gap-3.5 pt-6">
            <div>
              ...
              <Textarea id="rescue-desc" required value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="Describe the situation: where you found the cat, its condition, any hazards nearby…" rows={4} className="mt-1.5" />
            </div>
            <div>
              <Label htmlFor="rescue-notes">Cat condition notes</Label>
              <Textarea id="rescue-notes" value={form.cat_condition_notes} onChange={(e) => set('cat_condition_notes', e.target.value)} placeholder="Injuries, behavior, approximate age, color…" rows={3} className="mt-1.5" />
            </div>
          </CardContent>
        </Card>
