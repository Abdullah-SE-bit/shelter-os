...
export default function LostAlertDetailPage() {
  ...
  if (loading) return <div className="p-8"><LoadingSpinner size="lg" text="Loading lost cat report…" /></div>;
  if (!alert) return (
    <div className="flex flex-col items-center gap-3 p-16 text-center">
      <Frown className="size-12 text-muted-foreground" />
      <h2 className="font-display text-xl font-bold text-foreground">Lost cat report not found</h2>
      <Button variant="secondary" asChild><Link to="/lost-found">Back to Lost &amp; Found</Link></Button>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-[720px] px-4 py-6 sm:px-6">
      <PageHeader title="Lost cat report" description={alert.title} backTo="/lost-found" backLabel="Lost & Found" />
        ...
        <div className="flex flex-col gap-4">
          ...
          {(alert.contact_phone || alert.contact_email) && (
            <div className="rounded-xl border border-info/25 bg-info/10 p-4">
              <h3 className="mb-2 text-xs font-bold tracking-wide text-info uppercase">Seen this cat? Contact the owner</h3>
          ...
          <div className="flex flex-wrap gap-2.5">
            <Button className="min-w-[180px] flex-1" asChild><Link to={`/lost-found/matches/${alert.id}`}><Link2 className="size-4" />View possible matches{alert.match_count > 0 ? ` (${alert.match_count})` : ''}</Link></Button>
          ...
          {!isOwner && alert.status === 'ACTIVE' && (
            <div className="text-[13px] text-muted-foreground">Think you found this cat? <Link to="/lost-found/found" className="font-bold text-primary">Report a found cat →</Link></div>
          )}
        </div>
      </div>

      <ConfirmDialog open={resolveOpen} title="Mark as resolved?" message="This closes the lost-cat report and any pending match suggestions. Do this once the cat is back safe."
