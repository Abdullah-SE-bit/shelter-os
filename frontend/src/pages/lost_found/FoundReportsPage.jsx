...
export default function FoundReportsPage() {
  ...
  const { data, loading, refetch } = useApi(() => lostFoundApi.listFound({ page, page_size: pageSize, status: statusFilter, q: search || undefined }), null, [page, statusFilter, search]);
  ...
  const handleSave = async (e) => {
    ...
    try {
      const breedGuess = form.breed_choice === BREED_DONT_KNOW ? form.breed_describe.trim() : form.breed_choice;
      ...
      setAddOpen(false); setForm(EMPTY_FORM); setPhotos([]);
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6">
      <div className="relative mb-6 flex flex-wrap items-center justify-between gap-4 overflow-hidden rounded-xl bg-gradient-to-br from-success to-[#4A8A42] px-6 py-6 text-white sm:px-8">
        ...
        <div>
          <h1 className="font-display text-[26px] font-bold">Found cat reports</h1>
          <p className="mt-1 text-sm opacity-85">Found a cat? Report it — we'll compare it with lost-cat reports, or a shelter can take it in.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" className="border-white/30 bg-white/18 text-white hover:bg-white/28 hover:text-white" asChild><Link to="/lost-found"><Search className="size-4" />Lost cats</Link></Button>
          <Button variant="secondary" onClick={() => setAddOpen(true)}><Plus className="size-4" />Report found cat</Button>
        </div>
      </div>
      ...
      {!loading && reports.length === 0 && (
        <EmptyState icon={ClipboardList} title="No found cat reports" message="No one has reported a found cat here yet. If you find one, report it!"
          action={<Button onClick={() => setAddOpen(true)}><Plus className="size-4" />Report found cat</Button>} />
      )}

      {!loading && reports.length > 0 && (
        <div className="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {reports.map((rep) => (
            <Link key={rep.id} to={`/lost-found/found/${rep.id}`} className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
              <div className="relative h-[160px] overflow-hidden bg-surface-muted">
                <img src={(rep.photos && rep.photos[0]) || CAT_PLACEHOLDER} alt="Found cat" className="size-full object-cover" onError={(e) => { e.currentTarget.src = CAT_PLACEHOLDER; }} />
        ...
        <form id="found-form" onSubmit={handleSave} className="flex flex-col gap-3.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <NativeSelect value={form.breed_choice} onChange={(e) => set('breed_choice', e.target.value)} className="mt-1.5">
                ...
                {breeds.map((breed) => <option key={breed.id} value={breed.display_label}>{breed.display_label}</option>)}
          ...
          {form.breed_choice === BREED_DONT_KNOW && (
            <div>
              ...
              <p className="mt-1.5 text-xs text-muted-foreground">No worries if you're unsure — describe what the cat looks like and it still helps with matching.</p>
            </div>
          )}
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">Location helps match with nearby lost cats — but it's optional.</p>
          </div>
          ...
          <div className="rounded-lg bg-surface-muted p-3.5">
            ...
            {form.is_lost_cat && (
              <div className="mt-2.5">
                <Label>Match it to a lost-cat report (optional)</Label>
                ...
                <p className="mt-1.5 text-xs text-muted-foreground">Either way, we'll compare the details and show you likely matches after you submit.</p>
              </div>
            )}
          </div>
