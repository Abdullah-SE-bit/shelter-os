...
export default function CreateLostAlertPage() {
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-6 sm:px-6">
      <PageHeader title="Report lost cat" description="Help us find your missing companion" backTo="/lost-found" backLabel="Lost & Found" />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Camera className="size-4" />Photos of your cat</CardTitle></CardHeader>
          <CardContent>
            ...
            {photos.length < MAX_PHOTOS && (
              <label className="flex min-h-[120px] cursor-pointer flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-destructive/30 bg-gradient-to-br from-destructive/5 to-surface-muted text-center text-muted-foreground">
                ...
                <span className="text-xs">Don't panic — a photo helps us find your cat</span>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label>Cat's name</Label><Input value={form.cat_name} onChange={(e) => set('cat_name', e.target.value)} placeholder="e.g. Whiskers" className="mt-1.5" /></div>
              <div>
                <Label>Breed</Label>
                <NativeSelect value={form.breed} onChange={(e) => set('breed', e.target.value)} className="mt-1.5">
                  <option value="">Select breed</option>
                  {breeds.map((breed) => <option key={breed.id} value={breed.id}>{breed.display_label}</option>)}
                </NativeSelect>
              </div>
            </div>
            ...
            <div><Label>Description *</Label><Textarea required value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="Describe your cat: color, markings, size, collar…" rows={4} className="mt-1.5" /></div>
            <div><Label>Behavioral notes</Label><Textarea value={form.behavioral_notes} onChange={(e) => set('behavioral_notes', e.target.value)} placeholder="Is your cat shy, friendly, responds to name…" rows={2} className="mt-1.5" /></div>
